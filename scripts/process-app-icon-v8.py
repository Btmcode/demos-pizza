#!/usr/bin/env python3
"""
Demos Pizza — App icon v8.
%2 daha büyük + ortalanmış.
"""
...
def zoom_content(img, zoom_pct=7):
    """%7 toplam zoom (önceki %5 + %2 daha)"""
    if zoom_pct <= 0:
        return img
    w, h = img.size
    crop_amount_w = int(w * zoom_pct / 100 / 2)
    crop_amount_h = int(h * zoom_pct / 100 / 2)
    cropped = img.crop((crop_amount_w, crop_amount_h, w - crop_amount_w, h - crop_amount_h))
...
def main():
    print("İkon %2 daha büyük + ortalanmış (toplam %7 zoom)...\n")
    
    src = Image.open(SRC)
    cropped = crop_to_content(src)
    square = center_crop_to_square(cropped)
    zoomed = zoom_content(square, zoom_pct=7)  # %5 + %2 = %7
    ...
    print("\n✅ İkon %2 büyütüldü + ortalandı (toplam %7 zoom)")
